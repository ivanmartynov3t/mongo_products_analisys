...
**Note:** Candidate 1's score below is the Stage 6 corrected value (5.67, not the original 5.00) — see its card for the correction. Candidate 18's score below is the **Stage 8 corrected value (4.67, not the original 7.00)** — see its card for the correction; this changes its shortlist standing materially, noted in the table. Table order reflects original Stage 2 ranking to keep Stage 3/4's "top 5" selection traceable; see [Stage 6/8 summary](#stage-6-re-ranking-after-fact-verification) for the corrected overall order.
...
| 1 | 18 | Slack/Teams/Jira/PagerDuty webhook notifications | 4 | 4 | 3 | 3 | ~~2~~→3 | ~~7.00~~ → **4.67** | ✅ *(carried from Stage 3/4; corrected score would likely not have cleared the funnel — see Stage 8 note on its card)* |
...
| 6 | 16 | Deeper Vector Search tooling | 4 | 3 | 3 | 4 | 3 | 4.67 | cut — rank 6 at Stage 2; now tied with Candidate 18's corrected score (see funnel note) |
...
| 2 | 19 — PII classification/discovery | 6.50 (7.00 after Stage 3, corrected back to 6.50 in Stage 4 — see [06](06-verification-notes.md)) | recommended |
| 3 | 1 — AI query/index advisor | **5.67** (corrected in Stage 6, was 5.00) | now understood as a gap-closer, not a lead-widener |
| 4 | 9 — QE/CSFLE key-vault UI | 5.00 | |
| 5 | 18 — Webhook notifications | ~~7.00~~ → **4.67** (corrected in Stage 8 — see its card) | dropped from rank 2 to rank 5/6 (tied with cut Candidate 16) once the false "notifications already exist" premise was caught |

**Stage 8 addendum:** the ranking above reflects both the Stage 6 (`Candidate 1`) and Stage 8 (`Candidate 18`) score corrections. Candidate 18's drop from rank 2 to a tie for rank 5 is the most consequential single correction in this research — see its full explanation on its card and in [06-verification-notes.md's Stage 8 addendum](06-verification-notes.md#stage-8--2026-07-31-dictionary-gap-analysis-sync-addendum).
...
### Candidate 18: Slack/Teams/Jira/PagerDuty webhook notifications — `PROP-webhook-notify` — **⚠️ corrected in Stage 8, score revised down materially**
**Classification:** ❔ Parity-Unverified · 🏠 Portfolio-Port (via `GOV-003`, 3T Lens — for the webhook/channel concept specifically; the Desktop-side notification substrate itself is Net-New, see below) · 🔗 Dictionary-Tracked · Status: **Shortlisted** *(carried forward from Stage 3/4; see the Stage 8 note on what a re-run of the funnel would likely have done)*

- Feature area: F-SCHED/F-GOV — proven concept elsewhere in the 3T family (`GOV-003`, built into 3T Lens)
- Description: Add webhook-based notification channels (Slack, Teams, Jira, PagerDuty) to the Task Scheduler so task success/failure/warning alerts land where teams already work.
- Segment fit: both
...
#### ⚠️ Stage 8 correction: the "alongside its current email/in-app notifications" premise was never checked against source, and it's wrong

Every prior version of this card (Stages 1-7) described the target state as "add webhooks *alongside* Task Scheduler's existing email/in-app notifications." That "existing" claim was asserted, never verified against source — and the 2026-07-31 Studio 3T Desktop source-code re-audit found it's false: **`SCHED-notifications` and `SCHED-email` are both confirmed absent.** No SMTP/email-sending code and no in-app notification/alert mechanism exists anywhere in the scheduler subsystem (`t3/tasks`, `t3/tasks/gui`, `t3/taskmanager`) — a scheduled task's success or failure is only visible if a user is actively looking at the Task Scheduler UI (or the transient `BackgroundOperation` progress indicator) at the time it runs. This changes the shape of the candidate from "bolt a new channel onto a working pipeline" to "build the entire notification pipeline, webhooks included, from scratch" — disclosed here rather than left standing, per this research's own citation-discipline rule (see Stage 6/7).

**Practical effect:** Build Effort moves from 2 (Small) to 3 (Medium) — still within the effort ceiling, and still meaningfully de-risked by 3T Lens's `GOV-003` giving a proven design to port (channel config UI, payload templates, delivery retry) rather than inventing the concept from zero, but the Desktop-side trigger wiring (hooking `TaskScheduleManager`'s success/failure/warning points into *any* outbound notification, not just a webhook) is genuinely new engineering, not additive to an existing feature. Severity's rationale is corrected too: the workaround today isn't "check a less-convenient channel than the one you'd prefer" — it's "there is no alert of any kind; you must have the app open and be looking at it," which is a materially worse baseline than the original card described, even though it doesn't cross the rubric's Severity-4 bar ("forces switching to a secondary/competitor tool") since Studio 3T users don't switch tools over this, they just don't get notified.

#### Stage 7 re-mining result: no new evidence found, and that gap is itself the key finding

A full-text search for webhook/Slack/alert-channel language across all 21 `google_research` files (not just the one already cited) turned up nothing beyond `studio-3t-missing-integrations`' original recommendation. This is disclosed rather than papered over: **this candidate's entire evidentiary base is one Tier B recommendation plus one Tier A in-house precedent** — one of the weaker evidence bases in the long-list. Combined with the Stage 8 Build Effort correction, this candidate's profile shifts from "cheap and broadly useful" to "moderate cost, still broadly useful, not urgently demanded" — a materially less compelling case than its original 7.00 tied-for-first score suggested.

#### How each metric was calculated

- **Evidence Strength: 4** — calculated from: a *proven, already-shipping* implementation in a sibling product (3T Lens's `GOV-003`, which is a stronger evidentiary category than a mere proposal, since it demonstrates the concept works and is buildable) plus one independent Tier B recommendation naming Desktop specifically. Capped below 5 because, per the re-mining above, no third source or direct customer request exists. *(Unaffected by the Stage 8 correction — this axis was never about whether notifications already exist on Desktop.)*
- **Reach: 4** — calculated from: every Task Scheduler user benefits (a broad existing user base per `SCHED-` sub-feature usage), and the channel list (Slack, Teams, Jira, PagerDuty) spans both small-team (Slack) and enterprise-ops (PagerDuty, Jira) tooling — hence both segments, not capped at "enterprise only." *(Unaffected.)*
- **Severity: 3** — *rationale corrected in Stage 8, score unchanged.* Today's model is not "email/in-app only" as previously stated — it is **no outbound notification of any kind**; a user must have the Task Scheduler UI open to learn a task failed. This is worse than the original framing but still lands at 3, not 4, because no source claims Studio 3T customers abandon the product over this specifically (the rubric's bar for 4).
- **Competitive Urgency: 3** — calculated from general devtools-industry framing (webhook-based alerting is standard in CI/CD and ops tooling broadly) rather than any specific named MongoDB-GUI competitor — hence "emerging trend" territory (3) rather than "named competitor has it" (which would require a citation this research does not have). *(Unaffected.)*
- **Build Effort: 2 → 3** *(corrected in Stage 8)* — the original "additive to an existing pipeline" premise was wrong (see the correction above): `SCHED-notifications`/`SCHED-email` don't exist, so this is build-the-substrate-plus-one-channel, not add-a-channel-to-existing-substrate. Kept at 3 (Medium, still within the effort ceiling) rather than 4 because 3T Lens's `GOV-003` provides a proven design to port, meaningfully de-risking the build relative to a from-scratch invention.
- **Priority Score (corrected): (4 + 4 + 3 + 3) / 3 = 14 / 3 = 4.67`** — down from the 7.00 that stood unchanged through Stages 1-7. **This is the single largest score correction of any candidate in this research**, and it happened only because a routine sync against an unrelated, independently-run source-code audit caught a premise that had been asserted, not verified, from Stage 1 onward.

#### Stage 8 note: what this would likely have meant for the funnel, had it been caught earlier

At 4.67, Candidate 18 now ties with Candidate 16 (Deeper Vector Search tooling, cut at rank 6) and sits below Candidate 9 (5.00, rank 5) — meaning a Stage 2 pass run with the corrected Build Effort would likely **not** have placed this in the original top-5 funnel outright; it would have been a borderline call at best. Because Stages 3/4's deep-dive and adversarial-verification work on this candidate already happened (and remains valid research in its own right — the community-search and competitor-search findings in [05](05-shortlist-deepdive.md)/[06](06-verification-notes.md) don't depend on the Build Effort number), this research does not retroactively erase that work or silently re-run the funnel. Instead: the corrected score is the one the final memo must use, and the memo should note plainly that this candidate's placement in the original shortlist rested partly on an unverified assumption that didn't hold up.
...
### Candidate 11: Visual ERD/JSON-Schema editor/validation-rule authoring UI (the F-SCHEMA cluster) — `PROP-schema-erd-cluster` — **⚠️ scope narrowed in Stage 8**
**Classification:** ⚔️ Gap-Close (vs. Compass, VisuaLeaf) · 🆕 Net-New · 🔗 Dictionary-Tracked (**9** IDs — corrected 2026-07-31, was 13) · Status: **Deferred (separate future decision)**

- Feature area: F-SCHEMA — **9** IDs confirmed absent across the entire 3T portfolio (`SCHEMA-geo-analysis`, `SCHEMA-json-editor`, `SCHEMA-bson-types`, `SCHEMA-field-constraints`, `SCHEMA-designer-canvas`, `SCHEMA-designer-auto`, `SCHEMA-designer-links`, `SCHEMA-designer-color`, `SCHEMA-designer-portability`). **Corrected 2026-07-31 (Stage 8):** this cluster was originally 13 IDs; the 2026-07-31 Studio 3T Desktop source-code re-audit found Studio 3T already has a real, mature validator authoring/deployment feature (`SCHEMA-validation-model`, `-strictness`, `-ui`, `SCHEMA-deploy-validator`, plus `SCHEMA-validation-limits`) — the prior "confirmed absent" status for those 4-5 IDs was a documentation gap, not a product gap. They're removed from this cluster's dictionary link.
- Description: A visual entity-relationship diagram designer (infinite canvas, auto-detected relationships), a tree-based JSON Schema editor with full BSON type/constraint support, and geo-field map-backed analysis. *(Narrowed 2026-07-31 — validation-rule authoring/deployment is no longer part of this description; Studio 3T Desktop already has it, see above.)*
- Segment fit: both
...
- Evidence Strength: **5** — this is the highest-confidence Tier A finding in this entire research effort, corroborated by 2 independent competitor research passes. **Corrected framing (Stage 8):** the "two different competitors" point no longer includes Compass's validation UI as a differentiator, since Studio 3T Desktop now has validator authoring/deployment itself (see above) — the checked comparison data (`reports/comparisons/low-level-feature-comparison.md`) instead shows Compass ahead only on `SCHEMA-geo-analysis` (map-backed geo query drawing) within the narrowed 9-ID cluster, and VisuaLeaf ahead on the other 8. Evidence Strength stays at 5 on the strength of the Tier A confirmed-absent finding itself, which is unchanged by the narrowing.
- Reach: **4** — schema design/validation is broadly relevant across backend devs, data engineers, DBAs, and consultants. *(Unaffected.)*
- Severity: **3** — real competitive exposure (this is literally the narrowest thing the whole 3T portfolio still can't do that direct competitors can, now that validator authoring is resolved), but not blocking for existing Studio 3T users, who have workarounds (hand-written `$jsonSchema`, or switching to Compass/VisuaLeaf for that specific task). *(Unaffected in substance; rationale reworded to reflect the narrower cluster.)*
- Competitive Urgency: **4** — still an established market expectation among 2 direct competitors on the narrowed 9-ID cluster (VisuaLeaf leads on 8 of 9; Compass leads on the remaining one, `SCHEMA-geo-analysis`). *(Unaffected — the corrected comparison-table check above confirms the same 2-competitor urgency picture holds even after narrowing.)*
- Build Effort: **4** — 9 distinct sub-feature IDs (canvas, auto-diagramming, relationship detection, layouts, portability, JSON-tree editor, BSON types, field constraints, geo analysis) remains a large surface area even after removing the 4-5 validator IDs, since the removed IDs were comparatively the simpler, form-based part of the original 13; the visual-canvas/relationship-detection/JSON-tree-editor work that remains is, if anything, the harder half. Stays at Large, exceeding the medium ceiling.
- Priority Score: (5+4+3+4)/4 = **4.00** — arithmetically unchanged; the Stage 8 narrowing corrects *what* this candidate is (9 IDs, not 13) and *why* it scores as it does, not the score itself.
...
The ~54 unverified + remaining present-elsewhere IDs not absorbed into a headline candidate (full corrected list in [Stage 1](03-candidate-longlist.md#atomic-dictionary-gaps--triaged-not-individually-carded); was ~78 before the **Stage 8, 2026-07-31** sync with the Desktop source-code re-audit — see [01-research-plan.md](01-research-plan.md#stage-8--2026-07-31-sync-with-the-desktop-source-code-re-audit)) were reviewed as a group rather than individually carded. Collectively: most score low on Reach (single-control/format items, e.g. `QUERY-max-time`, `SHELL-minimap`) and low on Severity (no corroborating pain evidence in any tier). None individually approaches the shortlist threshold. This conclusion is unaffected by the Stage 8 resolution of roughly half the original list to confirmed-present (those items simply left the gap universe entirely, they didn't newly qualify as candidates) — the remainder is, if anything, more concentrated in genuinely low-Reach/low-Severity items now that the higher-profile resolved items (F-GOV's RBAC tree/inheritance, F-AGG's pagination/timer-cancel/stage-count, F-IDX's real-time monitoring) are out of the pool. **Recommendation:** treat these as a standing backlog for routine roadmap grooming, separate from this "widen the lead" decision — re-visiting them is a different, smaller-grained kind of decision than the one this research is answering.
