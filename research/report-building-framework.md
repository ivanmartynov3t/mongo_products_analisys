> **Status: archival, not currently executable (disposition set 2026-09-04).** Nothing in this methodology is non-compliant — it's pure computation over the metric records `gather-metrics-instructions.md`'s compliant pipeline produces (Studio 3T's own forum + public competitor changelogs). It's inapplicable **today** for a narrower reason: the framework's own quality gate (`N_f ≥ 5` mentions per feature, Step 5) requires a larger, sustained sample than the current pilot has. [`reports/voice-of-customer-metrics.md`](../reports/voice-of-customer-metrics.md)'s 7 records (one source, ~1 record per feature) don't clear that bar, which is exactly why that pilot reports per-record findings only and explicitly does not run the Public FPI formula below. Apply this framework once a scaled-up run of the compliant gathering pipeline produces enough volume per feature to normalize meaningfully — re-check the `N_f ≥ 5` gate before running it, don't lower the threshold to make a small sample fit.
