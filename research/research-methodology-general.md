> **Status: archival, not currently executable (disposition set 2026-09-04).** This methodology requires resources this environment does not have and cannot reach compliantly on its own: live developer interviews and conjoint/MaxDiff survey panels (Section 2 onward), CRM pipeline data (Salesforce/HubSpot), support-ticket systems (Zendesk/Jira), in-app telemetry instrumentation, and moderated usability benchmarking sessions. None of these are a web-research or ToS problem — they require organizational access (a live CRM connection, a support-ticket export, an analytics SDK already shipped in the product, a research-ops function to run interviews/surveys) that would need to be granted from outside this repository. Kept for reference if that access becomes available. The compliant, executable subset of *this repository's* customer-feedback research is the public-forum-and-changelog pipeline in [`gather-metrics-instructions.md`](gather-metrics-instructions.md) and [`report-building-framework.md`](report-building-framework.md), demonstrated in [`../reports/voice-of-customer-metrics.md`](../reports/voice-of-customer-metrics.md) — that is a narrower methodology than this document describes, not an execution of it. Do not attempt to execute the steps below as written without first re-checking they don't violate a data source's Terms of Service or require credentials this repository does not have.
