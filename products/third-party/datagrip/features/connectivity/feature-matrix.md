...
| CONN-test-steps | Connection test | Confirmed (a single connection test); Unverified (step-by-step diagnosis) | S5: "To do this, click the Test Connection link at the bottom of the connection details section." Checked https://www.jetbrains.com/help/datagrip/mongodb.html on 2026-09-26 in its silo copy only (captured 2026-09-11, not re-fetched live); no step-by-step breakdown (network, SSH, TLS, auth, permissions) documented. | — | Unverified (step-by-step) | S5 | Plan 09 pilot (silo candidate). The dictionary asks for step-by-step validation; only a one-shot test is documented, which is silence on the steps, not absence. |
