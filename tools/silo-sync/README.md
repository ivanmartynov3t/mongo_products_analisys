...
| 5 | evidence gaps | [`evidence-gaps`](../evidence-gaps/README.md) | `reports/evidence-gaps.md` | prod_info_silo#47 |
| 6 | taxonomy check | [`taxonomy-reconcile`](../taxonomy-reconcile/README.md) `--check` | nothing | #18 |
| 7 | scope triggers | [`scope-triggers`](../scope-triggers/README.md) | nothing | #18 |
| 8 | pin check | [`silo-pins`](../silo-pins/README.md) `check` | nothing | — |

Steps 2–5 read the silo from git objects at `origin/main`. Step 6 reads the silo's working tree, so it runs only when the silo checkout is a clean `origin/main`; otherwise it is reported as failed with the command to fix it. The script never changes the silo checkout.
