...
def test_snapshot(tmp: Path) -> None:
    ...
    check("catalog entries", a["catalog_entries"], 4)
    check("catalog entries that are not stored files are source files", a["catalog_source_files"], 2)
    ...
    check("seeds from config", (a["entry_urls"], a["sitemap_urls"], a["github_repos"]),
          (["https://alpha.example/"], ["https://alpha.example/sitemap.xml"], ["https://github.com/org/alpha"]))
    ...
    check("silo tree unchanged", tree_hashes(silo), silo_before)
