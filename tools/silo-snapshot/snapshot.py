...
def load_silo(root: Path, ref: str, data_dir: str, config_path: str, catalog_path: str) -> dict:
    ...
    config = yaml.safe_load(blobs[f"{sha}:{config_path}"] or "{}") or {}
    catalog = json.loads(blobs[f"{sha}:{catalog_path}"] or "{}")
    ...
    for k, ps in symbols.items():
        ...
        for p in ps:
            try:
                stamps.append(str(json.loads(texts[f"{sha}:{p}"]).get("scraped_at", "")))
    ...
    return {
        ...
        "products": config.get("products") or [], "catalog": catalog,
...
def build_snapshot(silo: dict, repo: Path, products_dir: str) -> dict:
    ...
    for cat, slug in sorted(keys):
        ...
        stored = set(silo["web"].get((cat, slug), [])) | set(silo["repo_docs"].get((cat, slug), []))
        entries = by_product.get(slug, [])
        entries = [e for e in entries if e.get("category", cat) == cat]
        source_files = sum(1 for e in entries if f"{silo['data_dir']}/{e.get('path', '')}" not in stored)
        ...
        rows.append({
            ...
            "github_repos": sorted(cfg.get("github_repos") or []),
...
def render_markdown(snap: dict, output_json: str) -> str:
    ...
    for cat, title, with_track in (("3t", "3T products", True), ("third-party", "Third-party products", False)):
        rows = [r for r in snap["products"] if r["category"] == cat]
        if not rows:
            continue
        head = ["Product", "Silo slug"] + (["Track", "Status"] if with_track else []) + \
               ["Web pages", "Repo docs", "Catalog", "Web retrieved", "Repo scraped", "Seeds", "Analysis"]
        out += [f"## {title}", "", "| " + " | ".join(head) + " |", "|" + "---|" * len(head)]
        for r in rows:
            seeds = len(r["entry_urls"]) + len(r["sitemap_urls"]) + len(r["github_repos"])
            ...
            cells += [str(r["web_pages"]), str(r["repo_docs"]),
                      ...
                      _cell(r["web_retrieved"]), _cell(r["repo_scraped"]), str(seeds),
    ...
    out += ["", "## Seed URLs", "", "From the silo's `config/products.yaml`: entry URLs, sitemaps and GitHub repositories.", ""]
    for r in snap["products"]:
        seeds = [*r["entry_urls"], *r["sitemap_urls"], *r["github_repos"]]
        if not seeds:
            continue
