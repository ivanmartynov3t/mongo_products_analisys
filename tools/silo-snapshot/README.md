...
Per silo product (every product in the silo's `config/products.yaml`, plus any data folder missing from it):
...
| Catalog (source files) | entries for the product in `data/catalog_index.json`; the bracket counts entries that are indexed source files rather than stored documents |
| Web retrieved | latest `updated_at` in the web pages' frontmatter |
| Repo scraped | `scraped_at` in `repo_symbols_and_strings.json` |
| Seeds | `entry_urls`, `sitemap_urls` and `github_repos` in `config/products.yaml` |
| Analysis | `products/<category>/<slug>/` in this repository, matched by name only |

Earlier hand-copied "docs" figures (for example in `docs/coverage-scope.md`) are the **catalog** count. The catalog lists only classified documents, so it can be smaller than web pages plus repo docs.
...
- **Deterministic.** Same silo commit and same product folders, same bytes out (tested).
- **Not evidence.** A count says what the silo holds, not what a product can do. It never becomes a ✅ or ❌ in a matrix.
- Seed URLs name GitHub repositories, some of them private. The snapshot lists their names only and copies no content from them.
