...
- When a source was read from `prod_info_silo`, pin the exact copy on its Source index line as ``silo: `data/<category>/<product>/<file>.md@<commit>` `` (commit SHA, 7+ hex). `tools/silo-pins` lists, checks and re-pins these; don't write silo copies as prose ("silo copy … at silo commit …").
