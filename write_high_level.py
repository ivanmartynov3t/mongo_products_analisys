content = r"""# High-Level Product Comparison
...
| Enterprise auth | ✅ 🆓 Kerberos, LDAP, AWS IAM, OIDC — all confirmed | 🔶 LDAP, AWS IAM confirmed; 🗺️ Kerberos + OIDC roadmap Q2 2026 <sup>[1](#fn-conn-1)</sup> | 🔒 🏢 Kerberos, LDAP, AWS IAM, OIDC — Ultimate edition only |
...
| Proxy | 🔶 SOCKS5 only | 📝 | ✅ 🎯 4 modes: Direct / App default / Custom HTTP / Custom SOCKS — per connection |
| Connection pool params | 📝 | ✅ Full set incl. Retry Reads + zlib compression | ✅ Full set incl. Azure idle-time workaround |
| Connection organization | 🔶 Favorites, sidebar search | ✅ 3-level hierarchy (Project → Environment → Connection); 🔒 Community max 3 <sup>[2](#fn-conn-2)</sup> | ✅ Folders + drag-and-drop; per-tab color coding |
| In-use encryption (QE/CSFLE) | ✅ 🎯 🔗 (key vault + KMS config) <sup>[3](#fn-conn-3)</sup> | ❌ | ❌ |
| Connection test validation | 📝 | ✅ 🎯 💡 6-step: Network/SSH/TLS/Auth/DB/Permissions | 📝 Not documented as multi-step |
...
> 🎯 **Compass only:** Multiple concurrent connections (1.44.0+); QE/CSFLE in-use encryption <sup>[3](#fn-conn-3)</sup>; required-access guide.
> 🎯 **VisuaLeaf only:** 6-step connection test wizard; URI export; DocumentDB/Cosmos DB roadmap Q2 2026 <sup>[1](#fn-conn-1)</sup>.
> 🎯 **Studio 3T only:** SSH Profiles; 4-mode proxy; read-only UI lock; team sharing; import from Robo 3T/NoSQLBooster/.uri.
>
> All three products cover core connectivity (standalone/RS/sharded/SRV, standard auth, TLS, SSH). Compass leads on in-use encryption and enterprise auth completeness. VisuaLeaf offers the best connection-test UX and uniquely plans DocumentDB/Cosmos compatibility. Studio 3T has the most advanced proxy, SSH, team-sharing, and credential-import capabilities.
...
| Multi-document update | 📝 | ✅ Table View Batch Update ($set/$unset/$inc/$push/$pull + preview) | ✅ Update Documents dialog (updateMany()); separate query+update tabs |
| Performance timer | 📝 | ✅ 🎯 💡 Amber at 2s; red at 5s | 📝 Not documented as visual timer |
| Cancel in-flight query | 📝 | ✅ 🎯 cancel button | 📝 |
| Explain view | ✅ | ✅ Exec plan, index usage, docs examined vs returned, index suggestions | ✅ 💡 Visual Explain across 5 entry points |
| Export to driver language | ✅ 8 languages (Java/Node/C#/Python/Ruby/Go/Rust/PHP) | 📝 | ✅ 9 languages (adds MongoDB Shell) |
| Undo/redo | 📝 | ✅ 🎯 Ctrl+Z / Ctrl+Shift+Z | 📝 |

> 🎯 **Compass only:** Collation in filter bar; max-time (60,000ms default).
> 🎯 **VisuaLeaf only:** Performance timer; cancel button; Run Find One / Run Count variants; 7 "open in" integrations; undo/redo; batch update from table view; EJSON copy.
> 🎯 **Studio 3T only:** Date tags (~20); Query Manager with 4 query types; "open in Aggregation Editor" converts filter → $match+$project.
>
> Compass is the only product with collation in the filter bar. VisuaLeaf has the most user-friendly query execution UX (timer, cancel, run-variants, undo/redo). Studio 3T's date tags and Query Manager are unique value adds for power users. VQB is absent in Compass; VisuaLeaf's is plan-gated; Studio 3T's is available on all editions.
...
| Stage toggle (enable/disable) | ✅ confirmed | ❓ Unknown/unverified <sup>[fn-agg-1](#fn-agg-1)</sup> | ✅ confirmed |
...
| Code generation | ✅ Major driver languages | ❓ Not documented <sup>[fn-agg-2](#fn-agg-2)</sup> | ✅ 9 languages; "Open in IntelliShell" button |
| Create MongoDB view | ✅ | ❓ Not documented <sup>[fn-agg-3](#fn-agg-3)</sup> | ✅ (requires MongoDB 3.4+) |
...
| Execution timer + cancel | 📝 | ✅ 🎯 💡 real-time timer + cancel | 📝 |

> 🎯 **Compass only:** Stage Wizard mode (GUI form for stage params).
> 🎯 **VisuaLeaf only:** 37+ stages (widest coverage); Form Mode with VQB for $match + accumulator builder for $group; chart builder from output; execution timer + cancel.
> 🎯 **Studio 3T only:** 9-language code gen; date tags in $match; switch collection mid-session; clipboard copy of full pipeline JSON.
>
> VisuaLeaf has the widest stage coverage and the most ergonomic per-stage editing (Form Mode). Studio 3T has the most powerful code-gen and integration options. Compass's Stage Wizard provides good accessibility for less-experienced users. Stage toggle availability is confirmed for Compass and Studio 3T but unverified in VisuaLeaf <sup>[fn-agg-1](#fn-agg-1)</sup>.
...
| Validation strictness (warn/error, strict/moderate) | ✅ confirmed | ❓ Unknown/unverified <sup>[fn-schema-2](#fn-schema-2)</sup> | ❌ N/A |
...
| View creation from schema tool | 📝 | 📝 | ✅ 🎯 db.createView() builder — all editions |
| Schema doc export | ✅ ⚠️ Multiple formats (fails if >1000 distinct fields) <sup>[fn-schema-1](#fn-schema-1)</sup> | ✅ $jsonSchema-compatible JSON | ✅ Word (.docx) or CSV with field names/types/probabilities |
| Explore docs by field presence | 📝 | 📝 | ✅ 🎯 right-click → opens Collection Tab with {$exists} filter |
| Rename field across all docs | 📝 | 📝 | ✅ 🎯 via schema tree outlier workflow |

> 🎯 **Compass only:** Geo field analysis with interactive map and geo-query generation.
> 🎯 **VisuaLeaf only:** Full JSON schema tree editor with BSON types; visual ERD designer with relationship detection.
> 🎯 **Studio 3T only:** Field statistics analytics (top values, date distributions); explore docs by field presence; rename field workflow; create view from schema.
>
> Compass and Studio 3T provide rich schema analytics (field probability, histograms). Compass adds geo analysis. VisuaLeaf uniquely offers a full JSON schema editor and a visual ERD designer with graph-theory relationship detection. Compass and VisuaLeaf have schema validator deploy; Studio 3T does not. Studio 3T has the most schema-to-data-repair workflows (explore, rename, view creation). ⚠️ Compass schema export fails if the collection has more than 1,000 distinct fields <sup>[fn-schema-1](#fn-schema-1)</sup>. ❓ VisuaLeaf validation strictness (warn/error, strict/moderate) is unverified <sup>[fn-schema-2](#fn-schema-2)</sup>.
...
| Dimension | MongoDB Compass | VisuaLeaf | Studio 3T |
| --- | --- | --- | --- |
| Index types supported | 🔶 single, compound, TTL, unique, sparse, partial, hidden (others not documented) | ✅ single, compound, text, wildcard, hashed, TTL, 2d, 2dsphere, partial, sparse, hidden | ✅ ⚠️ single, compound, text, wildcard, TTL, 2d, 2dsphere, geoHaystack (deprecated — no warning shown) <sup>[fn-idx-1](#fn-idx-1)</sup>, partial, sparse, hidden |
| Atlas Search / Vector Search | ✅ 🎯 🔗 (Atlas M10+ or local MongoDB 7.0+) <sup>[fn-idx-3](#fn-idx-3)</sup> | ❌ | ❌ |
| Advanced index options | 📝 | ✅ 💡 Partial Filter Expression (JSON editor), Wildcard Projection, Storage Engine config, Commit Quorum, Clustered index | 📝 Not documented as grouped options |
| Collation index options | 📝 | ✅ 💡 60+ ICU locales; Strength 1-5; Case First/Level/Numeric/Alternate/Backwards | 🔶 Locale + strength only |
...
| Hide/unhide index | ✅ | ❓ Unknown/unverified <sup>[fn-idx-2](#fn-idx-2)</sup> | ✅ (MongoDB 4.4+) |
...
| Real-time performance monitoring | ✅ 🎯 💡 mongostat/mongotop/currentOp; pause/play | 📝 | ❌ |
| Kill running operations | ✅ (requires killop privilege) | ✅ kill button per operation | ❌ |

> 🎯 **Compass only:** Atlas Search + Vector Search index creation <sup>[fn-idx-3](#fn-idx-3)</sup>; real-time perf monitoring (mongostat/mongotop/currentOp).
> 🎯 **VisuaLeaf only:** P50/P95/P99 profiler percentiles; 4 index recommendation types; profiler export; hashed index; 60+ ICU locale collation; Commit Quorum; clustered index; 4 quick-action templates.
> 🎯 **Studio 3T only:** Index copy-paste across connections; geoHaystack (unwarned deprecation) <sup>[fn-idx-1](#fn-idx-1)</sup>; Explain available in 5 contexts incl. SQL Query + IntelliShell.
>
> Compass exclusively supports Atlas Search and Vector Search <sup>[fn-idx-3](#fn-idx-3)</sup>. VisuaLeaf has the most complete index type coverage and the most sophisticated profiler (P50/P95/P99, 4 recommendation types, export). Studio 3T's Explain is available in the widest set of contexts. ❓ VisuaLeaf hide/unhide index is unverified <sup>[fn-idx-2](#fn-idx-2)</sup>. ⚠️ geoHaystack displayed by Studio 3T without deprecation warning is a confirmed bug <sup>[fn-idx-1](#fn-idx-1)</sup>.
...
| Incremental export with resume points | ❌ | ✅ 🎯 up to 5 historical resume points; does NOT track updates to existing docs <sup>[fn-transfer-1](#fn-transfer-1)</sup> |
...
| Export source granularity | 📝 | ✅ 🎯 6 sources: entire collection / view / find() result / aggregation result / current cursor / selected documents |

> 🎯 **VisuaLeaf only:** Pre-import JS transforms; document filter conditions; server-side $pipeline pre-export transforms.
> 🎯 **Studio 3T only:** SQL import/export, Excel export, cross-server MongoDB transfer; 5 import write modes; data masking; incremental export <sup>[fn-transfer-1](#fn-transfer-1)</sup>; 6 export source granularity options.
>
> Studio 3T has deeper export/import format coverage (SQL, Excel, cross-server MongoDB), more import modes, data masking, and incremental export. VisuaLeaf uniquely offers pre-import JS transforms, document filter conditions, and server-side $pipeline pre-export transforms — better for ETL scenarios. VisuaLeaf's plan limits (Community = 0 tasks) are more restrictive.
...
| Conversation turns for refinement | ✅ 🎯 prior turns used | 📝 |
...
> 🎯 **VisuaLeaf only:** 11 AI model choices; plain-English explanation always on; "Send sample data" privacy toggle; conversation turns for refinement; multiple named AI configs; AES-masked API key storage.
> 🎯 **Studio 3T only:** Local MCP server (10 tools, HTTP 127.0.0.1:27117); MCP client integrations (VS Code, Cursor, Claude Desktop); 59 MCP tools via 3T Lens; stt-cli + PII scanner.
>
> VisuaLeaf's AI focuses on in-product UX (11 model choices, plain-English explanations, privacy toggle, conversation turns). Studio 3T's AI extends outward via MCP — enabling AI agents in external tools (VS Code, Cursor, Claude Desktop) to operate against MongoDB through Studio 3T, which is a fundamentally different capability.
...
> 🎯 **Compass only:** Comprehensive policy enforcement (network policy, startup/CLI policy, AI controls with human approval gate, protect mode, isolated edition, telemetry config).
> 🎯 **VisuaLeaf only:** Visual RBAC role inheritance tree (Professional).
> 🎯 **Studio 3T only:** CDC pipeline (Kafka/Pub/Sub/HTTP) via 3TL Bridge; Kubernetes Helm chart; OIDC multi-provider; 3T Lens platform (59 MCP tools); 3T Access RBAC platform.
>
> Compass has the most comprehensive built-in policy enforcement (network policy, startup/CLI policy, AI controls, protect mode, isolated edition). VisuaLeaf has strong RBAC for teams (visual role tree, audit log) without requiring a separate platform. Studio 3T's governance scales via 3T Lens + 3T Access + 3TL Bridge for enterprise CDC, event streaming, and Kubernetes deployments.
...
| Multiple script units per task | 📝 | ✅ 🎯 multiple IntelliShell script units per task |
...
> 🎯 **VisuaLeaf only:** Timezone/DST-aware scheduling; richer execution config (retry/batch/concurrent/timeout/history); 5 task status states; Clone task action; SendGrid email notifications.
> 🎯 **Studio 3T only:** 7 task types (incl. Data Masking, Reschema, SQL Migration); multiple IntelliShell script units per task; compare/sync task with setup wizard + diff view + direction toggle.
>
> Studio 3T has more task types (7 vs. VisuaLeaf's fewer), unique multi-script-unit tasks, and a dedicated compare/sync workflow. VisuaLeaf has timezone/DST awareness, richer execution config (retry/batch/concurrent), 5 task status states, and Clone action. VisuaLeaf's Community plan cannot schedule any tasks (0 limit).
...
- 🎯 🗺️ **DocumentDB/Cosmos DB roadmap:** Only product with stated plans for DocumentDB 8.0+ and Cosmos DB 4.2+ compatibility (Q2 2026).
...
| Enterprise auth (Kerberos/LDAP/AWS/OIDC) | ✅ 🆓 Free (all confirmed) | 🔶 🆓 LDAP/AWS IAM free (others 🗺️ roadmap) | 🔒 🏢 Ultimate edition only |
...
| ⚠️ geoHaystack shown without deprecation warning | Studio 3T (bug) |
...
with open('reports/comparisons/high-level-product-comparison.md', 'w') as f:
    f.write(content)
print("High-level file written successfully")
