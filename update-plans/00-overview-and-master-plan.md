...
| 7 | `07-taxonomy-reconciliation.md` | Reconcile `feature-dictionary.md` with the `prod_info_silo` taxonomy (issue #15): verify shared definitions, decide every extra ID (feed upstream / retire / child-of / analysis-only), migrate living files, change the silo taxonomy, publish the mapping table for the weekly triage (#18). | Independent; unblocks #18 |
...
- 2026-09-04 — Plan 1 completed: 3 of 4 named reports were already corrected by a prior session; the one real remaining gap (AI opt-in-default note) was propagated into `reports/comparisons/high-level-product-comparison.md`. The pre-existing 77-vs-78 unverified-count cross-report inconsistency was found but deliberately left unresolved and flagged, per Plan 1's own scope (it needs new derivation, not transcription — Plan 3's job). See `01-apply-audit-corrections.md`'s own execution log.
- 2026-09-04 — Plan 2 completed: `next-feature-recommendation.md` link fixed by publishing the Stage 5 memo to its intended canonical path (`reports/next-feature-recommendation.md`) rather than a redirect; `.gitignore` added and `.DS_Store` untracked; `write_high_level.py` deleted (confirmed diverged from the hand-maintained comparison file); `visual-eaf`/"VisuaLeaf" naming mismatch documented as intentional (folder slug vs. brand name); stale product/matrix counts corrected in `README.md` and `reports/cumulative-report.md`. See `02-repo-hygiene-and-stale-content.md`'s own execution log.
- 2026-09-04 — Plan 4 completed in full: all 5 named competitors (DBeaver, DataGrip, Navicat, NoSQLBooster, TablePlus) now have `products/third-party/` entries (product-report + feature-matrix/feature-report per applicable Feature ID), folded into `feature-dictionary.md`'s coverage matrix, both comparison reports, and `products/third-party/README.md`. 16 new sub-feature IDs were minted across the effort, each justified against the full dictionary before being added (see `feature-dictionary.md`'s 2026-09-04 Changelog entries). A dictionary-first, classification-compliant approach (mint IDs before use; every claim labeled Confirmed/Roadmap/Unverified with no other vocabulary) was made an explicit non-negotiable section at the top of `04-extend-competitor-coverage.md` per an explicit instruction to make this clear in the plan document itself, not just in execution. A repo-wide link-resolution sweep across all 5 new folders found and fixed 4 broken cross-feature links in Navicat's files (wrong relative-path depth); no other broken links or undefined sub-feature IDs were found. Full per-product detail: `04-extend-competitor-coverage.md`'s own execution log.
- 2026-09-04 — Plan 6 completed: each of the three `research/` methodology documents received its own individually-justified disposition (not a blanket one) — `gather-metrics-instructions.md` rewritten as ToS-compliant, `report-building-framework.md` marked archival-pending-sample-size, `research-methodology-general.md` marked archival-resource-gated. `README.md`'s methodology-documents section updated to match. See `06-retire-or-rewrite-methodology-docs.md`'s own execution log.

- 2026-09-24 — Plan 7 completed (issue #15): taxonomy reconciled with `prod_info_silo`; 82 extra IDs decided (8 fed upstream, 20 retired as synonyms, 27 child-of, 27 analysis-only), silo taxonomy fixed and extended, mapping table published for #18. See `07-taxonomy-reconciliation.md`'s own execution log.

**All four one-time plans from this batch (1, 2, 4, 6) are now complete**, each landed as its own set of independently-revertible commits on branch `docs/one-time-repo-updates`. Plans 3 and 5 remain open-ended/recurring per their own definitions and were not in scope for this batch.
