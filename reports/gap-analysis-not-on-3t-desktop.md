...
This file has three sections, in order of severity:

1. **Confirmed absent from the whole 3T portfolio** — identical to the confirmed-absent list in the [companion report](gap-analysis-not-on-3t-products.md), since these feature areas (F-SCHEMA) aren't covered by any of the other five 3T products either. If no 3T product has it, Desktop doesn't have it.
2. **Present in the 3T portfolio, but not Desktop-native** — sub-features confirmed supported somewhere in the 3T product family, but delivered by 3T Explore, 3T MCP, 3T Lens, 3T Access, or 3TL Bridge as a *separate product/deployment*, not by the Desktop IDE itself. This is the section unique to this report — it doesn't exist in the companion report at all, because from a whole-portfolio view these are NOT gaps.
3. **Unverified** — same list as the companion report; these feature areas (F-CONN/QUERY/AGG/IDX/SCHED/SHELL/TRANSFER plus Desktop-native F-AI/F-GOV sub-features) are ones where the Desktop IDE is the only 3T product with that feature area at all, so "unverified for the portfolio" and "unverified for Desktop" are the same fact.

Data verified as of 2026-07-29 directly against every product's own `feature-matrix.md` files (see the [companion report's methodology note](gap-analysis-not-on-3t-products.md#methodology) for the corrections this uncovered).

## 1. Confirmed absent from the whole 3T portfolio (13)

All 13 are in **F-SCHEMA (Schema)** — see the [companion report](gap-analysis-not-on-3t-products.md#confirmed-absent--no-3t-product-supports-this-13) for the full table with competitor contrast. Sub-feature IDs: `SCHEMA-geo-analysis`, `SCHEMA-validation-model`, `SCHEMA-validation-ui`, `SCHEMA-validation-strictness`, `SCHEMA-deploy-validator`, `SCHEMA-json-editor`, `SCHEMA-bson-types`, `SCHEMA-field-constraints`, `SCHEMA-designer-canvas`, `SCHEMA-designer-auto`, `SCHEMA-designer-links`, `SCHEMA-designer-color`, `SCHEMA-designer-portability`.

## 2. Present in the 3T portfolio, but NOT on Studio 3T Desktop (21 rows / 17 unique sub-feature IDs)

These are **not gaps for 3T Software Labs as a company** — every one of them is confirmed supported by a named 3T product. They are gaps **for the Desktop IDE specifically**: a user who only has Studio 3T Desktop installed does not get these capabilities without adopting a separate product.

### F-AI — AI Features (3 IDs, via 3T MCP / 3T Explore)
...
### F-GOV — Governance & Security (14 IDs, via 3T Lens / 3T Access / 3TL Bridge / 3T Explore)
...
**Takeaway:** Studio 3T Desktop's own governance capability matrix is effectively empty of confirmed capabilities of its own — see the note below.

## 3. Unverified — not confirmed present or absent on Studio 3T Desktop (83)

Identical to the companion report's unverified list — see [gap-analysis-not-on-3t-products.md](gap-analysis-not-on-3t-products.md#unverified--not-confirmed-present-or-absent-on-any-3t-product-83) for the full breakdown by feature area (F-AI, F-GOV, F-CONN, F-IDX, F-QUERY, F-SCHED, F-SHELL, F-TRANSFER, F-AGG, F-SCHEMA, F-SQL).

## Known gap in this analysis: Desktop-native governance is under-documented, not confirmed absent

Eight governance sub-feature IDs are referenced by the comparison reports **against Studio 3T specifically** (not the split-out platform products) but have no dedicated capability matrix backing them in `products/3t/studio-3t/features/governance/feature-matrix.md` — that file's content was entirely about the five platform products before the 2026-07-29 split (see its [Scope note](../products/3t/studio-3t/features/governance/feature-matrix.md)). These are a **pre-existing documentation gap**, not confirmed-absent or confirmed-present, and are intentionally excluded from both sections 1 and 2 above:
